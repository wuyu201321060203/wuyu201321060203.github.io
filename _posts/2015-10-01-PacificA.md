...
因为primary只有在所有secondaries将请求添加进prepared list之后才可以通过采用移动committed point的方式将该请求插入committed list中，因此primary的committed list肯定是任何一个副本的前缀。同时，因为一个secondary只有在primary将一个请求添加进committed list后才会把同样的请求添加进committed list中，因此一个secondary上的committed list肯定是primary上committed list的前缀，这就存在一个复制协议必须要遵守的不变式，此不变式称为Commit Invariant。
