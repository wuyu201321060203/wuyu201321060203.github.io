...
上面基本阐述了CMake的主要用法，但这只是冰山一角，还有很多需要学习的。当你需要查询CMake哪个指令怎么用时用cmake --help-command XXX，查询CMake都提供了哪些指令用cmake --help-commands。另外还有4个小提醒：
...
- 在包含子目录的工程中，源码路径可以形成一棵以项目顶级目录为根节点的树，用户通过SET显示设置的变量遵循parent节点设置,它的孩子节点都能共享，但是兄弟节点之间无法共享的原则，如果要共享要么提升变量的作用域到parent节点中，SET(VAR1 , value1 , PARENT_SCOPE)，要么通过放到CMake的内部cache中作为全局共享变量，SET(VAR1 , value1 , CACHE INTERNAL "")
