...
因为n个互不相容的事件中任何一个发生的概率等于这n个事件发生的概率和，我们进行一定程度的放缩得到上述不等式。跳表高度超过i的概率要小于至少有一个节点具有i层的概率，这个值不超过P，我们定义一个常数c>1，因此跳表高度高于clogn的概率要小于1/(n^(c-1))，进而推出跳表高度h低于clogn的概率要大于1-1/(n^(c-1))。我们可以看出很大概率上，跳表高度h是O(logn)。事实上，层数产生的随机算法我们已经给出，如图Figure 5，我们定义Maxlevel的值为\log_{1/p}n，是O(logn)的。高度是O(logn)的情况下，我们继续分析跳表查找平均复杂度。
