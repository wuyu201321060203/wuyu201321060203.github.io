...
jemalloc是一个能够快速分配/回收内存，减少内存碎片，对多核友好，具有可伸缩性的内存分配器。为更好的支持多核内存分配的需要，jemalloc首次在FreeBSD中引入，用于取代之前的phkmalloc，而后被facebooke发扬光大，成为了业界流行的内存分配解决方案。
...
####jemalloc核心架构
 ...
 jemalloc采用多级内存分配，引入线程缓存tcache, 分配区arena来减少线程间锁的争用， 提高申请释放的效率和线程并发扩展性。它根据内存对象的大小把其分为small object, large object和huge object。
...
jemalloc的内存分配和释放的具体细节可以参考文章[^ 1]，这里不作详细介绍，jemalloc的内存分配就介绍完毕，这里做一个简单的总结：
