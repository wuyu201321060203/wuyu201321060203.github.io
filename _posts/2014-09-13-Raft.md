...
Raft比Paxos更易理解，同时也更方便用于实际分布式系统的构建。Raft既保证了一致性又有较好的性能。本文对Raft进行了粗浅的分析，参照官方论文的基础上结合了自己的理解进行讲解，如有错误之处，请及时指出并通知我，不胜感激。另外，对于集群配置变化中Raft使用的方法我并没有理解透，后面会对本文进行补充说明。