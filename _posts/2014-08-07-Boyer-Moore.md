...
* “好后缀”的位置以最后一个字符为准。假定"ABCDEF"中"EF"是个好后缀，则其位置为"F"的位置，即为5
* 如果“好后缀”在搜索串中不只出现过一次，那么移动步数为：好后缀位置 - 其在搜索串中上一次出现的位置。
* 如果"好后缀"在搜索串中只出现过一次，并且除最长的那个好后缀外（本身除外），如果有“好后缀”的“好后缀”（本文叫好后缀子缀）出现在头部（注意必须只能出现在头部），则移动位数为：好后缀位置 -  其好后缀子缀位置。如果没有好后缀子缀，则移动位数为：好后缀位置 - （-1）
